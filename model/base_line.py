...
class Base_Line():
    def __init__(self,model_params):
        ...
        self.list_size = model_params.list_size
    ...
    def _build_base_line_listwise(self):
        with tf.variable_scope('inputs') as inputs:
            self._ques = tf.placeholder(tf.int32,[None,self.list_size,self.ques_len],name='ques_point')
            self._ques_len = tf.placeholder(tf.float32,[None,self.list_size,self.ques_len],name='ques_len_point')
            self._ans = tf.placeholder(tf.int32,[None,self.list_size,self.ans_len],name='ans_point')
            self._ans_len = tf.placeholder(tf.float32,[None,self.list_size,self.ans_len],name='ans_len_point')

            self._ques_filter_len = tf.tile(tf.reshape(self._ans_len,[-1,self.list_size,1,self.ans_len]),[1,1,self.ques_len,1])
            self._ans_filter_len = tf.tile(tf.reshape(self._ques_len,[-1,self.list_size,1,self.ques_len]),[1,1,self.ans_len,1])

            self._ques_align_len = tf.tile(tf.reshape(self._ques_len,[-1,self.list_size,self.ques_len,1]),[1,1,1,self.hidden_dim])
            self._ans_align_len = tf.tile(tf.reshape(self._ans_len,[-1,self.list_size,self.ans_len,1]),[1,1,1,self.hidden_dim])

            self.p_label = tf.placeholder(tf.float32,[None,self.list_size])
            self.l_label = tf.placeholder(tf.float32,[None,self.list_size])
        with tf.name_scope('list_wise'):
            with tf.variable_scope('embedding_layer') as embedding_l:
                weights = np.load(self.embedding_file)
            ...
            with tf.variable_scope('cnn_feature') as cnn_l:
                ...
                ques_cnn = tf.concat([self.conv1d_listwise(cnn_ques,ques_aligned[:,i,:,:],keep_dims=True) for i in range(ques_aligned.shape[1])],axis=1)
                ans_cnn = tf.concat([self.conv1d_listwise(cnn_ans,ans_aligned[:,i,:,:],keep_dims=True) for i in range(ques_aligned.shape[1])],axis=1)
                print('ques_cnn:',ques_cnn.shape)
            with tf.variable_scope('output_layer') as out_l:
                ques_o1 = tf.layers.dense(ques_cnn,self.hidden_dim,activation=tf.tanh,name='q_out1')
                ans_o1 = tf.layers.dense(ans_cnn,self.hidden_dim,activation=tf.tanh,name='a_out1')
